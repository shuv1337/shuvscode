...
function getResourceUri(candidate: unknown): Uri | undefined {
	...
	const resourceUri = (candidate as SourceControlResourceState).resourceUri;
	return resourceUri instanceof Uri ? resourceUri : undefined;
}
...
export class CommandCenter {
	...
	@command('git.openHunkDiff')
	async openHunkDiff(arg?: Resource | Uri, ...resourceStates: SourceControlResourceState[]): Promise<void> {
		const resolveResource = (candidate: unknown): Resource | undefined => {
			if (candidate instanceof Resource) {
				return candidate;
			}

			const resourceUri = getResourceUri(candidate);
			return resourceUri ? this.getSCMResource(resourceUri) : undefined;
		};

		const resources = [...resourceStates, arg]
			.map(resolveResource)
			.filter(isDefined);
		...
		const verifiedHunkCommands = new Map<string, boolean>();

		for (const resource of resources) {
			const resourceUri = resource.resourceUri;

			if (!resourceUri) {
				window.showWarningMessage(l10n.t('Unable to open Hunk diff because the Git resource does not have a file path.'));
				continue;
			}
			...
			if (!repository) {
				window.showWarningMessage(l10n.t('Unable to open Hunk diff because no Git repository was found for "{0}".', path.basename(resourceUri.fsPath)));
				continue;
			}

			const relativeResourcePath = path.relative(repository.root, resourceUri.fsPath);

			if (!relativeResourcePath || relativeResourcePath.startsWith('..') || path.isAbsolute(relativeResourcePath)) {
				window.showWarningMessage(l10n.t('Unable to open Hunk diff because "{0}" is outside the Git repository.', resourceUri.fsPath));
				continue;
			}
			...
			if (canLaunch === undefined) {
				canLaunch = await canLaunchHunkCommand(hunkCommand);
				verifiedHunkCommands.set(hunkCommand, canLaunch);
			}

			if (!canLaunch) {
				window.showWarningMessage(l10n.t('Unable to launch Hunk using "{0}". Install Hunk or update the git.hunkCommand setting.', hunkCommand));
				continue;
			}

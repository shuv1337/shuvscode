...
class ResourceCommandResolver {
	...
	resolveDefaultCommand(resource: Resource): Command {
		const config = workspace.getConfiguration('git', Uri.file(this.repository.root));
		const openHunkDiffOnClick = config.get<boolean>('openHunkDiffOnClick', true);

		if (openHunkDiffOnClick) {
			return this.resolveHunkDiffCommand(resource);
		}
	...
	resolveHunkDiffCommand(resource: Resource): Command {
		return {
			command: 'git.openHunkDiff',
			title: l10n.t('Open Hunk Diff'),
			arguments: [resource]
		};
	}

...
- Browser control for this repo means controlling the running shuvscode Electron workbench, not opening a new Chrome window. Always start the built app with a dedicated CDP port, then run `agent-browser --session <fresh-name> --cdp <port> ...` against that port for every command.
- Before clicking or typing in a smoke test, confirm the target is shuvscode with `agent-browser --session <fresh-name> --cdp <port> get title` or `snapshot`. If the title/UA is `about:blank`, Chrome, or anything other than the shuvscode workbench, stop and relaunch shuvscode with a fresh profile and CDP port.
- Do not satisfy shuvscode Electron validation with the Chrome plugin, in-app Browser plugin, `agent-browser connect`, `xdg-open`, or commands that open a separate browser profile. Those are only acceptable if the user explicitly asks to debug generic Chrome rather than shuvscode.

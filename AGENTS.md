...
- Browser control for this repo means controlling the running shuvscode Electron workbench through its own CDP port, not opening a new Chrome window. Always start the built app with a dedicated `--remote-debugging-port=<port>`, then run `agent-browser --session <fresh-name> --cdp <port> ...` against that port for every command.
- Before clicking or typing in a smoke test, confirm the target is shuvscode with `agent-browser --session <fresh-name> --cdp <port> get title` or `snapshot`. If the title/UA is `about:blank`, Chrome, or anything other than the shuvscode workbench, stop immediately; do not interact with that target. Relaunch shuvscode with a fresh isolated profile and a fresh CDP port, then retry with direct `--cdp <port>` commands.
- Do not satisfy shuvscode Electron validation with the Chrome plugin, in-app Browser plugin, `agent-browser connect`, `xdg-open`, or commands that open a separate browser profile. If a tool path would create or control a standalone Chrome window, it is the wrong tool path for this repo unless the user explicitly asks to debug generic Chrome rather than shuvscode.
...
- Managed Zellij sessions can be created from the shuvscode extension host rather than an interactive terminal, so Zellij process launches must provide terminal defaults (`TERM=xterm-256color`, `COLORTERM=truecolor`) or panes may inherit no `TERM` and render p9k/ANSI colors incorrectly. Existing bad sessions need an explicit user-approved `zellij delete-session --force shuvscode-managed` or equivalent recreate; do not do this automatically.
